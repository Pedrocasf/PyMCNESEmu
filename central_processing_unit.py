...
class CPU:
    ...
    def __init__(self):
        ...
        self.opcode_table = {0x00:self.BRK,0x01:self.ORA(address=self.indexed_indirect()),0x02:self.STP,0x03:self.SLO(address=self.indexed_indirect()),0x04:self.NOP,0x05:self.ORA(address=self.zero_page()),0x06:self.ASL(address=zero_page())
                            ,0x07:self.SLO(address=self.zero_page()),0x08:self.PHP,0x09:self.ORA(address=self.immediate()),0x0A:self.ASL(address=self.accumulator),0x0B:self.ANC(address=self.immediate()),0x0C:self.NOP,0x0D:self.ORA(address=self.absolute())
                            ,0x0E:self.ASL(address=self.absolute()),0x0F:self.SLO(address=self.absolute()),0x10:self.BPL,0x11:self.ORA(adress=self.indirect_indexed()),0x12:self.STP,0x13:self.SLO(address=self.indirect_indexed()),0x14:self.NOP,
                            0x15:self.ORA(address=self.zero_page_x()),0x16:self.ASL(address=self.zero_page_x()),0x17:self.SLO(address=self.zero_page_x()),0x18:self.CLC,0x19:self.ORA(address=self.absolute_y()),0x1A:self.NOP,0x1B:self.SLO(address=absolute_y())
                            ,0x1C:self.NOP,0x1D:self.ORA(address=self.absolute_x()),0x1E:self.ASL(address=self.absolute_x()),0x1F:self.SLO(address=self.absolute_x()),0x20:self.JSR, 0x21:self.AND(address=self.indexed_indirect()),0x22:self.STP,
                            0x23:self.RLA(address=self.indexed_indirect()),0x24:self.BIT(address=self.zero_page()),0x25:self.AND(address=self.zero_page()),0x26:self.ROL(address=zero_page()),0x27:self.RLA(address=self.zero_page()),0x28:self.PLP
                            ,0x29:self.AND(address=self.immediate()),0x2A:self.ROL(address=self.accumulator),0x2B:self.ANC(address=self.immediate()),0x2C:self.BIT(address=self.absolute()),0x2D:self.AND(address=self.absolute()),0x2E:self.ROL(address=self.absolute())
                            ,0x2F:self.RLA(address=self.absolute()),0x30:self.BMI,0x31:self.AND(address=self.indirect_indexed()),0x32:self.STP,0x33:self.RLA(address=self.indirect_indexed()),0x34:self.NOP,0x35:self.AND(address=zero_page_x()),
                             0x36:self.ROL(address=self.zero_page_x()),0x37:self.RLA(address=self.zero_page_x()), 0x38:, 0x39:, 0x3A:, 0x3B:, 0x3C:, 0x3D:, 0x3E:, 0x3F:
